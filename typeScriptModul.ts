...
function HosegriadoSzint( nap1 : number, nap2 : number, nap3 : number) :number{
    ...
    let haromNapAtlaga : number = (nap1 + nap2 + nap3 ) / 3
    let homersekletkHarmasFok = 27
    let homersekletKettesFok = 25

    if (haromNapAtlaga >= homersekletkHarmasFok){ return 3}

    else if (haromNapAtlaga >= homersekletKettesFok) { return 2 }

    else if (haromNapAtlaga >= homersekletKettesFok || nap1 >= homersekletKettesFok || nap2 >= homersekletKettesFok || nap3 >= homersekletKettesFok) { return 1 }

    else { return 0 }
}

console.log(HosegriadoSzint(25,24,23))

function OszthatoSzamok(oszto : number , vizsgaltTomb : number[]){

    let maradekNelkulOszthatoSzamok : number[] = []

    vizsgaltTomb.forEach((vizsgaltSzam : number) => {
        if (vizsgaltSzam % oszto ===  0){
            maradekNelkulOszthatoSzamok.push(vizsgaltSzam)
        }
    })

    return maradekNelkulOszthatoSzamok.length
}

function Erettsegi(pontok: number[]): [number, number] {
    let osszpontszam: number = pontok.reduce((osszeg : number, pont : number) => osszeg + pont, 0);
    let jegy: number;
...
function LeetKod(vizsgaltSzoveg : string) : string {
    ...
    return vizsgaltSzoveg.replace(/[ioaeIOAE]/g, (karakter) => {
        switch (karakter) {
            case 'i': return '1';
            case 'I': return '1';
            case 'o': return '0';
...
const snookerInfo : string[] = ["52;Akani Sunny;Thaiföld;118500",
...
function objektumFeltolto(SnookeJatekosok :string[]) : SnookeJatekos[] {
    let snookerAdatok: SnookeJatekos[] = []
    SnookeJatekosok.forEach( jatekosAdatokEgyben => {
        let jatekosAdatok =  jatekosAdatokEgyben.split(';')
        snookerAdatok.push({
            helyezes: parseInt(jatekosAdatok[0]),
            nev: (jatekosAdatok[1]),
            orszag: (jatekosAdatok[2]),
            nyeremeny: parseInt(jatekosAdatok[3])
        })
    })
...
let snookerAdatok  = objektumFeltolto(snookerInfo)

console.log(snookerAdatok)

function LegtobbNyeremeny(snookerInfoObjects : SnookeJatekos[]) : number{
    let maxNyeremeny: number = 0
    snookerInfoObjects.forEach(snookerInfoObject => {
        (snookerInfoObject.nyeremeny > maxNyeremeny) ? maxNyeremeny = snookerInfoObject.nyeremeny : maxNyeremeny
    })

    return maxNyeremeny
}

console.log(LegtobbNyeremeny(snookerAdatok))